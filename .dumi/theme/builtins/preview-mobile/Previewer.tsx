...
function getPreviewSourceTabs(sources: IPreviewerComponentProps['sources']) {
  return Object.keys(sources).map((filename) => {
    const source = sources[filename];
    const type = getSourceType(filename, source);
    return {
      label: filename === '_' ? `index.${type}` : filename,
      key: filename,
    };
  });
}
      ...
      <div className="__dumi-default-previewer-source-wrapper">
        {!isSingleFile && (
          <Tabs
            ...
            items={getPreviewSourceTabs(props.sources)}
          ></Tabs>
        )}
